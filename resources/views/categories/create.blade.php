...
@section('content')
    <div class="container category-create-container">
        <div class="category-create-card">
            <h1 class="category-create-title">Добавить категорию</h1>
            ...
            @endif
            <form action="{{ route('categories.store') }}" method="POST" class="category-create-form">
                @csrf
                <div class="form-group">
                    <label for="name" class="form-label">Название категории</label>
                    <input type="text" name="name" class="form-control" value="{{ old('name') }}" required placeholder="Например: Салаты">
                </div>
                <button type="submit" class="btn btn-success category-create-btn">Добавить категорию</button>
            </form>
        </div>
    </div>
@endsection
